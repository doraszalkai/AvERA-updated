...
int empty_cells = 0;       // Üres cellák száma
int non_empty_cells = 0;   // Nem üres cellák száma
...
/*void kiiras(REAL** x)
{
	...
	//char filename[0x100];
    char filename[1024]; // JAVÍTVA 0x100-ról 1024-re!
...
void Log_write()
{
    ...
    //fprintf(LOGFILE, "%.15f\t%e\t%e\t%.15f\t%.15f\t%.15f\t%.15f\t%.10f\n", T*47.1482347621227, errmax, h*47.1482347621227, a, a_max/a-1, Hubble_param*20.7386814448645, Decel_param, Omega_m_eff, empty_cells, non_empty_cells);
    fprintf(LOGFILE, "%.15f\t%e\t%e\t%.15f\t%.15f\t%.15f\t%.15f\t%.10f\t%d\t%d\n", T*47.1482347621227, errmax, h*47.1482347621227, a, a_max/a-1, Hubble_param*20.7386814448645, Decel_param, Omega_m_eff, empty_cells, non_empty_cells);
